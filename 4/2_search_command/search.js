(function() {
  ...
  function toggleSearch() {
    ...
    if ($searchBox.classList.contains(visibleClass)) {
      $searchBox.classList.remove(visibleClass);
      stopFindInPage();
    }
    else {
      $searchBox.classList.add(visibleClass);
      $searchInput.focus();
    }
  }
  
  const {ipcRenderer} = require('electron');
  ipcRenderer.on('toggleSearch', toggleSearch);
})();
